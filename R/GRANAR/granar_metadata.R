...
batch_Gmeta <- function(path = "~/Thesis/2020-10 HydraulicViper/MECHA/cellsetdata/wide/"){
  ...
  for(j in fls){
    tmp_output <- granar_metadata(paste0(path,j))
    ...
    MXA <- tmp_output$value[tmp_output$io == "output" & tmp_output$param == "metaxylem" & tmp_output$type == "layer_area"]
    RXA <- tmp_output$value[tmp_output$io == "output" & tmp_output$param == "all" & tmp_output$type == "layer_area"]
    CCA <- tmp_output$value[tmp_output$io == "output" & tmp_output$param == "cortex" & tmp_output$type == "layer_area"]
    AA <- tmp_output$value[tmp_output$io == "output" & tmp_output$param == "aerenchyma" & tmp_output$type == "layer_area"]
    EE <- tmp_output$value[tmp_output$io == "output" & tmp_output$param == "exodermis" & tmp_output$type == "layer_area"]+tmp_output$value[tmp_output$io == "output" & tmp_output$param == "epidermis" & tmp_output$type == "layer_area"]
    if(length(AA) == 0){AA = 0}
    TCA = CCA + AA + EE
    ...
    nPX = nX*ratio
    PXA = tmp_output$value[tmp_output$io == "output" & tmp_output$param == "xylem" & tmp_output$type == "layer_area"]
    a = PXA_1*1000^2
