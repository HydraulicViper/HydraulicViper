...
Fast_GM <- function(model_list, type = "kr"){
  descr = c("radius", "var_stele", "var_xylem", "aerenchyma", "kw", "kAQP", "kpl", "thickness")
  ...
  svm <- model_list[[1]]
  rf <- model_list[[2]]
  Sa <- fast(FAST, svm, rf, type = type)
  Sa$apo = 1
  
  svm <- model_list[[3]]
  rf <- model_list[[4]]
  Sb <- fast(FAST, svm, rf, type = type)
  Sb$apo = 2
  
  svm <- model_list[[5]]
  rf <- model_list[[6]]
  Sc <- fast(FAST, svm, rf, type = type)
  Sc$apo = 3
...
fast <- function (FAST, svm, rf, type = "kr"){
  design <- FAST$X
  kr = (predict(svm, design)+ predict(rf, design))/2
  if(type == "Kr"){
    kr = kr * 2*pi*design$radius/10 * 0.02
  }
